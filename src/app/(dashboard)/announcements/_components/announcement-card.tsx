        ...
        <CardTitle>
          <div className="flex justify-between items-start">
            ...
            <Badge
              ...
              variant={announcement.status === 'published' ? 'green' : 'yellow'}
            >
              {announcement.status}
            </Badge>
          </div>
          <p className="mt-2 text-[13px] text-muted-foreground font-normal">
            Posted By {announcement?.created_by_user?.first_name}{' '}
      ...
      <CardContent className="px-0 pb-0 relative flex-grow">
        <ul className="divide-y divide-gray-50">
          ...
          <li className="py-2 px-5 text-[13px]">
            ...
            <Show>
              <Show.If
                condition={(announcement?.departments?.length ?? 0) > 0}
          ...
          <li className="py-2 px-5 text-[13px]">
            ...
            <Show as={Fragment}>
              <Show.If condition={(announcement?.users?.length ?? 0) > 0}>{formatUsers()}</Show.If>
              <Show.Else as="p">No users added</Show.Else>
            </Show>
          </li>
        </ul>
      </CardContent>
